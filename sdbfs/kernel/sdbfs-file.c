...
static ssize_t sdbfs_read(struct file *f, char __user *buf,
...
static ssize_t sdbfs_write(struct file *f, const char __user *buf,
				 size_t count, loff_t *offp)
{
	struct inode *ino = f->f_dentry->d_inode;
	struct sdbfs_dev *sd;
	...
	while (count) {
		...
		if (copy_from_user(kbuf, buf, n))
			return -EFAULT;
		/* FIXME: error checking */
		i = sd->ops->write(sd, start + done, kbuf, n);
		if (i < 0) {
			if (done)
				return done;
			return i;
		...
		count -= i;
		if (i != n) {
			/* Partial write: done for this time */
			break;
		}
	}
	*offp += done;
...
const struct file_operations sdbfs_iomem_fops = {
	.read		= sdbfs_read,
	.write		= sdbfs_write,
};
